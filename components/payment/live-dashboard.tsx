...
export const LiveDashboard = () => {
  ...
  return (
    <div ref={containerRef} className="container mx-auto px-4 lg:px-8 overflow-hidden py-20 min-h-screen flex flex-col justify-center">
          ...
          <div className="mb-16 sm:mb-20 lg:mb-32">
            <div className="flex flex-col lg:flex-row items-center gap-12 lg:gap-16 max-w-7xl mx-auto px-4 lg:px-0">
              ...
              >
                ...
                <h3 className="text-4xl lg:text-7xl font-bold mb-6 font-display leading-[1.1] tracking-tight">
            ...
            <motion.div 
              ...
              className="mt-16 w-full max-w-7xl mx-auto px-4 sm:px-0 transform-gpu will-change-transform"
            >
              <Card className="p-8 sm:p-12 border-slate-100 shadow-[0_40px_80px_-15px_rgba(0,0,0,0.1)] bg-white relative rounded-[3rem] overflow-hidden group/table">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-12 gap-6 relative z-10">
                  <div>
                    <h4 className="font-black text-3xl text-slate-900 tracking-tight">Top Performing SKUs</h4>
                    ...
                    <Table className="min-w-[600px]">
                      <TableHeader>
                        <TableRow className="border-slate-100 hover:bg-transparent">
                          <TableHead className="text-xs font-black text-slate-400 uppercase tracking-widest h-14">SKU Inventory</TableHead>
                          <TableHead className="text-xs font-black text-slate-400 uppercase tracking-widest h-14">Gross Sales</TableHead>
                          <TableHead className="text-xs font-black text-slate-400 uppercase tracking-widest h-14">Adj. Charges</TableHead>
                          <TableHead className="text-right text-xs font-black text-slate-400 uppercase tracking-widest h-14">Net Contribution</TableHead>
                        </TableRow>
                      </TableHeader>
                      <TableBody>
                        {[
                          ...
                          <TableRow key={i} className="border-slate-50 hover:bg-slate-50/50 transition-colors group/row">
                            <TableCell className="py-5">
                              <p className="font-black text-slate-900 text-sm group-hover/row:text-primary transition-colors">{row.sku}</p>
                            </TableCell>
                            <TableCell className="font-bold text-slate-600 text-sm">{row.sales}</TableCell>
                            <TableCell className="font-bold text-slate-600 text-sm">{row.charges}</TableCell>
                            <TableCell className="text-right">
                              <div className="inline-flex flex-col items-end">
                                <span className={`font-black text-sm ${row.color}`}>{row.profit}</span>
                                <span className="text-[10px] font-bold text-slate-400 uppercase">{row.trend} vs last week</span>
                              </div>
                            </TableCell>
                          </TableRow>
                        ))}
                  ...
                  <div className="mt-12 grid grid-cols-1 lg:grid-cols-2 gap-8 relative z-10">
                    <div className="p-8 rounded-[2rem] bg-slate-50/50 border border-slate-100 hover:border-violet-200 transition-colors duration-500">
                      <div className="flex items-center justify-between mb-8">
                        <div>
                          <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-1">Weekly Performance</p>
                          <p className="text-xl font-black text-slate-900">Sales Velocity</p>
                        </div>
                      ...
                      <div className="flex items-end gap-3 h-32 px-2">
                        {[40, 70, 45, 90, 65, 80, 55].map((h, i) => (
                          <motion.div 
                            ...
                            className="flex-1 bg-gradient-to-t from-violet-600 to-indigo-500 rounded-xl hover:from-violet-500 hover:to-indigo-400 transition-all cursor-pointer relative group/bar shadow-[0_4px_12px_rgba(124,58,237,0.2)]"
                          >
                            <motion.div 
                              initial={{ opacity: 0, y: 10 }}
                              whileHover={{ opacity: 1, y: 0 }}
                              className="absolute -top-10 left-1/2 -translate-x-1/2 bg-slate-900 text-white px-3 py-1.5 rounded-lg text-xs font-black whitespace-nowrap shadow-xl pointer-events-none"
                            >
                              ₹{h}k
                            </motion.div>
                          </motion.div>
                      ...
                      <div className="mt-6 flex justify-between px-2 text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                        <span>Mon</span><span>Tue</span><span>Wed</span><span>Thu</span><span>Fri</span><span>Sat</span><span>Sun</span>
                      </div>
                    </div>

                    <div className="p-8 rounded-[2rem] bg-slate-50/50 border border-slate-100 hover:border-emerald-200 transition-colors duration-500 flex items-center gap-8">
                      <div className="relative w-32 h-32 shrink-0 group/circle">
                        <svg className="w-full h-full -rotate-90 drop-shadow-xl" viewBox="0 0 32 32">
                          <circle cx="16" cy="16" r="14" fill="transparent" stroke="rgba(0,0,0,0.03)" strokeWidth="3" />
                          <motion.circle 
                            cx="16" cy="16" r="14" fill="transparent" stroke="#8b5cf6" strokeWidth="3" 
                          ...
                          />
                          <motion.circle 
                            cx="16" cy="16" r="14" fill="transparent" stroke="#10b981" strokeWidth="3" 
                            initial={{ strokeDasharray: "0 100" }}
                        ...
                        <div className="absolute inset-0 flex flex-col items-center justify-center">
                          <motion.p 
                            ...
                            className="text-slate-900 font-black text-xl leading-none"
                          >85%</motion.p>
                          <p className="text-[10px] text-slate-400 font-black mt-1 uppercase tracking-widest">ROI</p>
                        </div>
                      </div>
                      <div className="flex-grow">
                        <p className="text-xs font-black text-slate-500 uppercase tracking-[0.2em] mb-4">Capital Efficiency</p>
                          ...
                          {[
                            ...
                            <div key={i} className="space-y-2">
                              <div className="flex items-center justify-between">
                                <p className="text-xs text-slate-900 font-black">{item.label}</p>
                                <p className="text-xs text-slate-500 font-bold">{item.value}</p>
                              </div>
